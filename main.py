...
def process_dashboard_data(data, target_lang):
    ...
    if 'wf_words' in data:
        translated_data['wf_words'] = [translate_text(word, target_lang) for word in data['wf_words']]
    
    # Traducir países/ciudades
    if 'geo_countries' in data:
        translated_data['geo_countries'] = [translate_text(country, target_lang) for country in data['geo_countries']]
    
    # Copiar datos numéricos sin traducir
    for key in ['wf_counts', 'geo_counts', 'timeline_years', 'timeline_counts', 'drilldown_data']:
        if key in data:
    ...
    if 'stories' in data:
        ...
        for story in data['stories']:
            ...
            for field in ['title', 'summary', 'motive', 'travel_duration', 'return_plans', 'destination_city', 'destination_country']:
            ...
            if 'mentions' in story and story['mentions']:
                translated_story['mentions'] = [translate_text(m, target_lang) for m in story['mentions'] if m]
            
            # Traducir métodos de viaje
            if 'methods' in story and story['methods']:
                translated_story['methods'] = [translate_text(m, target_lang) for m in story['methods'] if m]
...
def change_password():
    current_lang = get_current_language()
    return render_template('changePassword.html', languages=LANGUAGES, current_lang=current_lang)
...
def data_exploration():
    ...
    wf_words_en = ["Work", "Family", "Hope", "Journey", "Future", "Freedom", "Opportunity", "America", "Dream", "Letters"]
    ...
    if current_lang == 'es':
        ...
        wf_words = ["Trabajo", "Familia", "Esperanza", "Viaje", "Futuro", "Libertad", "Oportunidad", "América", "Sueño", "Cartas"]
...
def upload_tool():
    ...
    try:
        ...
        for fname in os.listdir(app.config['UPLOAD_IMAGE_FOLDER']):
            if os.path.isfile(os.path.join(app.config['UPLOAD_IMAGE_FOLDER'], fname)) and allowed_file(fname, ALLOWED_IMAGE_EXTENSIONS):
                image_files.append(os.path.join('images', fname))
                
        for fname in os.listdir(app.config['UPLOAD_TEXT_FOLDER']):
            if os.path.isfile(os.path.join(app.config['UPLOAD_TEXT_FOLDER'], fname)) and allowed_file(fname, ALLOWED_TEXT_EXTENSIONS):
...
def admin_create_story():
    ...
    return render_template('admin_story_form.html', 
                         action='create',
                         languages=LANGUAGES, 
                         current_lang=current_lang)
...
def admin_admins():
    ...
    try:
        ...
        cur.execute("SELECT id_admin, email, created_at FROM admins ORDER BY created_at DESC")
        admins = cur.fetchall()
    ...
    if request.method == 'POST':
        try:
            ...
            if cur.fetchone():
                ...
                return render_template('admin_admin_form.html', 
                                     action='create',
                                     languages=LANGUAGES, 
                                     current_lang=current_lang)
            
            # Insertar nuevo administrador
            cur.execute("INSERT INTO admins (email, password) VALUES (%s, %s)", (email, password))
...
def admin_edit_admin(admin_id):
    ...
    if request.method == 'POST':
        try:
            ...
            cur.execute("SELECT id_admin FROM admins WHERE email=%s AND id_admin!=%s", (email, admin_id))
            if cur.fetchone():
                flash('Email already exists!', 'danger')
            ...
            if password:
                cur.execute("UPDATE admins SET email=%s, password=%s WHERE id_admin=%s", (email, password, admin_id))
            else:
                cur.execute("UPDATE admins SET email=%s WHERE id_admin=%s", (email, admin_id))
